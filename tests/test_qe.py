...
def test_read():
    ...
    for key in ideal:
        if key == 'ATOMIC_SPECIES':
            ideal[key] = aton.interface.qe.normalize_atomic_species(ideal[key])
        elif key in ['CELL_PARAMETERS', 'CELL_PARAMETERS_out']:
            ideal[key] = aton.interface.qe.normalize_cell_parameters(ideal[key])
        elif key in ['ATOMIC_POSITIONS', 'ATOMIC_POSITIONS_out']:
            ideal[key] = aton.interface.qe.normalize_atomic_positions(ideal[key])
...
def test_add_atom():
    ideal_positions = [
        ...
        'O   0.0  0.0  0.0',
        'Cl  1.0  1.0  1.0']
    ideal_positions = aton.interface.qe.normalize_atomic_positions(ideal_positions)
...
def test_normalize_cell_params():
    ...
    normalized_params = aton.interface.qe.normalize_cell_parameters(cell_params)
...
def test_normalize_atomic_positions():
    ...
    ideal_positions = [
        ...
        '  I   5.000000000000000   0.000000000000000   0.000000000000000',
        '  C   0.000000000000000   5.000000000000000   0.000000000000000']
    normalized_positions = aton.interface.qe.normalize_atomic_positions(atomic_positions)
...
def test_normalize_atomic_species():
    ...
    normalized_species = aton.interface.qe.normalize_atomic_species(atomic_species)
